		...
		>
			...
			>
				<li
					...
					:class="[`${prefix}carousel-item`, { [prop.activeClass]: _i === activeIndex, [prop.cloneClass]: useClone && _i >= realTotal }]"
...
<script setup lang="ts" generic="T extends {}">
			...
			onStart: () => {
				...
				if (!_isBack) {
					...
					for (const [_i, $el] of $items.value.entries()) {
						if (_i !== oldIndex.value && _i !== activeIndex.value) $el.style.display = 'none';
					}
					$items.value[activeIndex.value].style.display = '';
	...
	const repositionEffectMap = {
		slide: (_$el: HTMLElement, _index: number, _total: number) => {
			...
			if (_index < 2 || _index === _total) _$el.style.display = '';
		},
		...
		cover: (_$el: HTMLElement, _index: number, _total: number) => {
			...
			if (_index === 0 || _index === _total) _$el.style.display = '';
		},
		strip: (_$el: HTMLElement, _index: number) => {
			_$el.style.transform = `translateX(${_index === 0 ? 0 : -100}%)`;
			if (_index < 2) _$el.style.display = '';
		},
		// 모션 없음
		none: () => {},
	};
