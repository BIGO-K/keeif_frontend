<template>
	...
	<div class="m...info-ship">
		...
		<dropdown>
			<template #button>
				<i class="ico_ship"></i><strong>{{ date(averageArrivalProbability.average_date, 'MM월 DD일(ddd)') }}</strong>
				도착 확률<strong>{{ averageArrivalProbability.probability }}<sub>%</sub></strong>
			</template>
			<template #content>
				<template
					v-for="arrivalProbability in shippingArrivalProbabilities"
					:key="arrivalProbability.average_date"
				>
					<p>
						<span>{{ date(arrivalProbability.average_date, 'MM월 DD일(ddd)') }}도착확률</span>
		...
		<p
			v-if="exportableMessage !== '시 이전 결제 시 당일 출고됩니다.'"
			class="text_note"
		>
			{{ exportableMessage }}
		</p>
...
<script setup lang="ts">
	...
	const props = defineProps<{
		shippingArrivalProbabilities: GoodsDeliveryShippingArrivalProbability[],
		exportableMessage: string
	}>();
	const { date } = useFormatter();

	const averageArrivalProbability = computed(() => {
		if (props.shippingArrivalProbabilities.length > 1) {
			return props.shippingArrivalProbabilities[1];
		}

		return props.shippingArrivalProbabilities[0];
