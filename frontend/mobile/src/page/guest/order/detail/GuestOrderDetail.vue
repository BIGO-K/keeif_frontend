				...
				<template #content>
					<div class="mm_order-item">
						...
						<mm-link
							...
							:to="{ name: 'GuestOrderDetail', params: { id: order.original_order_id } }"
						>
									...
									<table>
										<tbody>
											...
											<tr>
												...
												<td>
													<p class="text_price mm_text-variable">
														<strong>{{ number(cancelDetail.data.refund_info.refund_price) }}</strong>
													</p>
												</td>
											</tr>
										</tbody>
								...
								<section>
									...
									<template v-if="returnDetail.data.retrieve_invoice">
										...
										<button
											...
											@click="bomShipNumberUpdate(
												returnDetail.id,
												'return',
												returnDetail.data.retrieve_invoice
											)"
										>
											<b>수정하기</b>
										</button>
									...
									>
										<button
											...
											@click="bomShipNumberCreate(returnDetail.id, 'return')"
										>
											<b>입력하기</b>
										</button>
									</div>
...
<script setup lang="ts">
	...
	function overlayAddress() {
		const component = defineAsyncComponent(() => import('@/page/guest/order/detail/address-change/GuestOrderDetailAddressChange.vue'));

		return openOverlay(component, {
			onClose: async (_value: { newAddress: MyShippingAddress }) => {
				const newAddress = _value.newAddress;
				if (!newAddress) {
					return;
				}

				await updateShippingAddress(props.orderId, newAddress);
