								...
								>
									<div class="mm_inline">
										...
										<a
											class="mm_btn T=2xs T=line T=ligher"
											href="#"
											@click.prevent="overlayMyInquirySingleUpdate(qna.id)"
										>
											<b>수정</b>
										</a>
									</div>
...
<script setup lang="ts">
	...
	/** 문의 composable */
	const { getQnaPaginator, destroyQna } = useInquiry();

	/** [1:1문의] 페이지네이터 */
	const paginator = ref<Paginator<Qna>>();

	/**
	 * [1:1문의] 수정 모달 OPEN
	 * @param id
	 */
	function overlayMyInquirySingleUpdate(id: number) {
		const component = defineAsyncComponent(() => import('@/page/mypage/inquiry/single/update/MyInquirySingleUpdate.vue'));
	...
	async function fetch(pageNumber: number = 1) {
		try {
			paginator.value = await getQnaPaginator(pageNumber);
		}
		catch (e) {
			useErrorAlert(e);
		}
	...
	async function remove(id: number) {
		bom.confirm('1:1 문의를 삭제하시겠습니까?', async (flag) => {
			if (!flag) {
				return;
			}
			...
			try {
				await destroyQna(id);
				bom.alert('1:1 문의가 삭제되었습니다.', async () => {
