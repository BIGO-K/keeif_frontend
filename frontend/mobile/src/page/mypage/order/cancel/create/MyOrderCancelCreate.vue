...
<script setup lang="ts">
	...
	/** my 주문 composable */
	const { defaultRefundInfo, getCancelReasons, getCancelables, getCancelRefundInfo, getRefundCalculations, applyCancel } = useMyOrder();

	const { get: getMyRefundAccount, getBankCodes } = useMyRefundAccount();

	const [cancelReasons, cancelableOrder, myRefundAccount] = await Promise.all([
		getCancelReasons(),
