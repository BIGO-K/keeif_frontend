		...
		<div class="mm_order-item">
			<div class="mm_order-item-seller">
				<div class="mm_order...seller-head">
					<p class="text_ship">
						<i class="ico_ship"></i>
						<span :class="{'text_price': switchReturnOrderPack.paid_shipping_price > 0}">
							<strong>{{ switchReturnOrderPack.paid_shipping_price === 0 ? '무료배송' : number(switchReturnOrderPack.paid_shipping_price) }}</strong>
						</span>
					</p>
				</div>
				<div class="mm_product-list T=sm">
					<ul>
						<li
							v-for="orderItem in switchReturnOrderPack.order_items"
							:key="orderItem.id"
						>
							<p class="text_seller">
...
<script setup lang="ts">
	...
	const switchReturnOrderPack = (await getReturnDetail(props.returnId, true)).pack;
</script>