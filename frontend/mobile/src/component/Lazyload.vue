<template>
	<component
		...
		ref="$ui"
		data-lazyload
		:class="[componentClasses, ratioClass]"
			...
			switch (_e.type) {
				...
				case 'load':
					...
					if (prop.isRatio) {
						...
						let value = 'S=image-square';
						if (ratio > 1) value = (ratio > 8) ? 'S=image-landscape-4x' : (ratio > 4) ? 'S=image-landscape-3x' : (ratio > 2) ? 'S=image-landscape-2x' : 'S=image-landscape';
						else if (ratio < 1) value = (ratio < 0.25) ? 'S=image-portrait-3x' : (ratio < 0.5) ? 'S=image-portrait-2x' : 'S=image-portrait';

						ratioClass.value = value;
					}
					...
					if (prop.tag === 'i') {
						...
						// display가 null이 아닐 때 높이가 없으면 강제 적용
						if ($ui.value.offsetParent != null && $ui.value.clientHeight === 0) $ui.value.style.paddingTop = `${$target.naturalHeight / $target.naturalWidth * 100}%`;// 높이가 없으면 비율로 적용
					}
